    ...
    useEffect(
        () => {
            fetch(`http://localhost:8088/employees/${employeeId}`)
                .then(res => res.json())
                .then(set) // Notice the implicit data that was returned being passed as a parameter
        },
    ...
    return (
        <>
            <section className="employee">
                ...
                <div className="employee__specialty">Specializing in {employee.specialty}</div>
            </section>
        </>
    )
}